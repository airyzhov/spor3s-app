// УЛЬТРА УМНЫЙ БОТ С ИСПРАВЛЕННЫМ ФОРМАТИРОВАНИЕМ
const { Telegraf } = require('telegraf');
const { createClient } = require('@supabase/supabase-js');
const axios = require('axios');
require('dotenv').config({ path: 'env.local' });

console.log('🧠 УЛЬТРА УМНЫЙ БОТ SPOR3S');
...
*Напишите что хотите заказать, и я помогу с оформлением!* ✨
    ...
    this.bot.on('text', async (ctx) => {
      ...
      try {
        ...
        // Вызываем AI API с правильным форматом
        console.log('🤖 Вызываем AI API...');
        const aiResponse = await this.callUltraSmartAI(message, context);
...
*Используйте /help для справки* 📚
  ...
  // УЛЬТРА УМНЫЙ вызов AI API
  async callUltraSmartAI(message, context) {
    try {

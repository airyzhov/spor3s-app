-- 🚀 БЕЗОПАСНАЯ МИГРАЦИЯ В НОВУЮ СИСТЕМУ УРОВНЕЙ
-- Этот скрипт проверяет текущее состояние и выполняет миграцию только при необходимости
...
-- 4. Миграция транзакций (только если их нет в новой системе)
...
-- 5. Миграция ежедневных активностей (только если их нет в новой системе)
...
WHERE NOT EXISTS (
    ...
    WHERE da.user_id = dc.user_id 
        AND da.date = DATE(dc.created_at)
)

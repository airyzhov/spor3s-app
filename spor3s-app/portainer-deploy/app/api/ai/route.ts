...
export async function POST(req: NextRequest) {
  const { message, context, source, user_id, products_prompt, telegram_id } = await req.json();
    ...
    try {
      ...
      if (existing?.id) {
        ...
        const { data: created, error: createErr } = await supabaseServer
          ...
          .select('id')
          .single();
        if (!createErr && created?.id) resolvedUserId = created.id;
      }
    } catch (e) {
...
📱 **Проверить баланс:** откройте приложение → раздел "Прогресс"`;
  ...
  if (context && context.length > 0) {
    ...
    context.forEach((msg: { role: string; content: string }, index: number) => {
  ...
  async function fetchCompletion(msgs: { role: string; content: string }[]) {
    try {
      ...
      if (products_prompt) {
        ...
        const categories = {
          ...
          'Комплекс': products.filter(p => p.name.includes('Комплекс') || p.name.includes('4 в 1'))
        };
        ...
        Object.entries(categories).forEach(([category, categoryProducts]) => {
          if (categoryProducts.length > 0) {
            ...
            categoryProducts.forEach(product => {
              productsInfo += `- ${product.id}: ${product.name} - ${product.price}₽\n`;
            });
            productsInfo += "\n";
          }
        });
      ...
      console.log('[AI API] Tags found:', { 
       ...
       const lastMsg = Array.isArray(msgs) ? msgs[msgs.length - 1] : null;
       ...
       const hasFormSpecification = hasContext && context.some((msg: { content?: string }) =>
         msg.content && (
...
Плюсы веган капсул что не нужно мерить и многим так привычнее 😊`;
       ...
       if (userMessage.includes('мухомор') || userMessage.includes('сон') || userMessage.includes('стресс')) {
        ...
        const hasAddContext = context && context.some((msg: { content?: string }) =>
          msg.content && (
            msg.content.includes('добавил') || 
            msg.content.includes('добавлен') || 
            msg.content.includes('добавляю')
          )
        );
...
Также у вас уже есть опыт приема добавок или начинаете впервые?`;
        ...
        const product = (findProduct(['мухомор', '30г']) || findProduct(['мухомор'])) as { id?: string; name?: string; price?: number } | undefined;
        const productPrice = product?.price ?? '600';
        const productTitle = product?.name || productName;
        const ezhProduct = findProduct(['ежовик']) as { price?: number } | undefined;
        const ezhPrice = ezhProduct?.price ?? '1200';
         return forceAddToCartTag(`Отлично! Добавил ${productTitle} в корзину за ${productPrice}₽. Он отлично помогает со сном и снимает стресс. Кстати, для лучшего эффекта рекомендую также Ежовик для улучшения памяти за ${ezhPrice}₽. Добавить его тоже? [add_to_cart:${productId}]`);
...
Также у вас уже есть опыт приема добавок или начинаете впервые?`;
        ...
        const product = (findProduct(['ежовик', 'капсул']) || findProduct(['ежовик'])) as { name?: string; price?: number } | undefined;
        const productTitle = product?.name || productName;
        const productPrice = product?.price ?? '1200';
        const kordiceps = findProduct(['кордицепс']) as { price?: number } | undefined;
        const kordicepsPrice = kordiceps?.price ?? '800';
         return forceAddToCartTag(`Добавил ${productTitle} в корзину за ${productPrice}₽! Он прекрасно помогает с памятью и концентрацией. Хотите также попробовать Кордицепс для энергии за ${kordicepsPrice}₽? [add_to_cart:${productId}]`);
        ...
        if (userMessage.includes('мухомор') || context && context.some((msg: { content?: string }) => msg.content && msg.content.includes('мухомор'))) {
          // Обработка мухомора
          let productId = 'mhm30';
          let productName = 'Мухомор 30г';
          ...
          const product = findProduct(['мухомор']) as { name?: string; price?: number } | undefined;
          const productTitle = product?.name || productName;
          const productPrice = product?.price ?? '1400';
          const ezhProduct = findProduct(['ежовик']) as { price?: number } | undefined;
          const ezhPrice = ezhProduct?.price ?? '1100';
          return forceAddToCartTag(`Отлично! Добавил ${productTitle} в корзину за ${productPrice}₽. Он отлично помогает со сном и снимает стресс. Кстати, для лучшего эффекта рекомендую также Ежовик для улучшения памяти за ${ezhPrice}₽. Добавить его тоже? [add_to_cart:${productId}]`);
        } else if (userMessage.includes('ежовик') || context && context.some((msg: { content?: string }) => msg.content && msg.content.includes('ежовик'))) {
          // Обработка ежовика
          let productId = 'ezh120k';
          ...
          const product = findProduct(['ежовик']) as { name?: string; price?: number } | undefined;
          const productTitle = product?.name || productName;
          const productPrice = product?.price ?? '1100';
          const kordProduct = findProduct(['кордицепс']) as { price?: number } | undefined;
          const kordPrice = kordProduct?.price ?? '800';
          return forceAddToCartTag(`Отлично! Добавил ${productTitle} в корзину за ${productPrice}₽. Он прекрасно помогает с памятью и концентрацией. Кстати, для лучшего эффекта рекомендую также Кордицепс для энергии за ${kordPrice}₽. Добавить его тоже? [add_to_cart:${productId}]`);
        }
      } else if (userMessage.includes('кордицепс') || userMessage.includes('энергия') || userMessage.includes('выносливость')) {
        const product = findProduct(['кордицепс']) as { id?: string; name?: string; price?: number } | undefined;
        const productId = product?.id || 'kor50';
        const productTitle = product?.name || 'Кордицепс 50г';
        const productPrice = product?.price ?? '800';
         return forceAddToCartTag(`Добавил ${productTitle} в корзину за ${productPrice}₽! Он отлично повышает энергию и выносливость. Хотите также попробовать Мухомор для сна за ${findProduct(['мухомор'])?.price || '600'}₽? [add_to_cart:${productId}]`);
      } else if (userMessage.includes('цистозира') || userMessage.includes('щитовидка') || userMessage.includes('щитовидная')) {
        const product = findProduct(['цистозира']) as { id?: string; name?: string; price?: number } | undefined;
        const productId = product?.id || 'ci30';
        const productTitle = product?.name || 'Цистозира 30г';
        const productPrice = product?.price ?? '500';
         return forceAddToCartTag(`Добавил ${productTitle} в корзину за ${productPrice}₽! Он отлично поддерживает здоровье щитовидной железы. Хотите также попробовать Ежовик для памяти за ${findProduct(['ежовик'])?.price || '1200'}₽? [add_to_cart:${productId}]`);
        ...
        const product = findProduct(['комплекс', '4 в 1']) as { name?: string; price?: number } | undefined;
        const productTitle = product?.name || productName;
        const productPrice = product?.price ?? '2000';
         return forceAddToCartTag(`Добавил ${productTitle} в корзину за ${productPrice}₽! Это комплексное решение для всех ваших потребностей. Хотите оформить заказ? [add_to_cart:${productId}]`);

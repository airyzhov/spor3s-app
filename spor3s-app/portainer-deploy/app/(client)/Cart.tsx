...
export default function Cart({ products = [], setStep }: CartProps) {
  ...
  const getCartItem = (id: string) => safeCart.find(item => item.id === id);
                  ...
                  {modalProduct?.image ? (
                    <img 
                      ...
                      onError={(e) => {
                        ...
                        target.style.display = 'none';
                        if (target.parentNode instanceof HTMLElement) {
                          target.parentNode.innerHTML = `<div style="display: flex; align-items: center; justifyContent: center; width: 100%; height: 100%; background: #f0f0f0; color: #999; font-size: 48px;">🍄</div>`;
                        }
                      }}
                    />
                  ) : (

...
- Цель месяца: 4 отчёта за календарный месяц → ровно один бонус 50 SC. Идемпотентность — по `source_type='month_goal'` внутри запроса, ограниченного текущим календарным месяцем. **Не** через `source_id`: колонка `sc_transactions.source_id` имеет тип uuid, строка вида `"2026-08"` туда не пишется (вставка падает молча, `creditSC` ошибку не проверяет).

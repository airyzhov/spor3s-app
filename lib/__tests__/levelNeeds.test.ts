...
describe('описания наград', () => {
  ...
  // Тексты владельца 25.09: у Легенды вместо мерча, встреч и трекинга — авторский курс
  it('у Мастера — бонусные товары для практик, у Легенды — авторский курс', () => {
    expect(benefits('master')).toContain('Бонусные товары для практик в магазине: травы, благовония, чай');
    expect(benefits('legend')).toContain('Доступ к авторскому курсу «Практики для развития внимания и снятия тревожности»');
    expect(benefits('legend')).not.toMatch(/Мерч|Личные встречи|Живой трекинг/);
  });
});
